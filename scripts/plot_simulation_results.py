...
class ResultPlotter:
    ...
    def plot_mismatch_pos_finder(self, save_fpath: Path) -> None:
        ...
        for file in self.input_files:
            ...
            ax.plot(result_dict[step]['x'], result_dict[step]['psi'], label=r'$\Psi=(V-U)/2x^2$',
                    ls='-', color='#006699', marker='o', markersize=5)
            
            ax.axvline(0.023, label=r'$\max \Psi=$'+f'{x_max_psi}', color="#8C1B3D", ls='--')
